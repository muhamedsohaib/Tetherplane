...
The relay serves both `/.well-known/oauth-protected-resource` and the resource-path-specific metadata URL (normally `/.well-known/oauth-protected-resource/mcp`). Metadata URLs come from configuration, never forwarded Host headers. In OAuth mode, initialization without an Authorization header creates an anonymous discovery session. It permits only the initialized notification, ping and tool listing; all six descriptors include top-level and `_meta` OAuth security schemes. Every unauthenticated tool call on an existing session returns an error result with `mcp/www_authenticate` and performs no execution or routing, including local schema lookup.

A valid OAuth bearer binds the anonymous session to its authenticated account, client and principal. Subsequent requests must authenticate as that exact identity; a session ID alone grants no authority. Invalid credentials never fall back to anonymous discovery: initialization and discovery return HTTP 401, while tool calls on an existing session receive the authentication error result for relinking. Anonymous GET and DELETE requests are rejected. Static bearer mode continues to require authentication before initialization and session creation.
